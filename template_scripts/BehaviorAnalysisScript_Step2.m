...
traj1 = getMovTraj(trajFile1, TRK, CODE_CALIB,'digit2trk',1);

% For digit 2
trajFile2 = ([BASEPATH 'Data_Analyzed/' SUB '/Behavior/' SUB '_' EXPER_SESSION '_' EXPER_COND '_trajectories2.mat']);
traj2 = getMovTraj(trajFile2, TRK, CODE_CALIB,'digit2trk',2);
%% Set trials
...
% washout
numWash = getNumTrialsPerScore(trialIdxs, wash);
...
if useJAABA
    ...
    for itrial = 1:length(movIdx.nbase) %1:numTrials
        grabStart.nbase(1,itrial) = grab.t0{1,movIdx.nbase(itrial)}{1,1}(1);
    end
    for itrial = 1:length(movIdx.npert) %1:numTrials
        grabStart.npert(1,itrial) = grab.t0{1,movIdx.npert(itrial)}{1,1}(1);
    end
    for itrial = 1:length(movIdx.nwash) %1:numTrials
        grabStart.nwash(1,itrial) = grab.t0{1,movIdx.nwash(itrial)}{1,1}(1);
    end
