...
if expt_type==1
    ...
    ind_pos = [1 1+find(diff(ind{1})>5e4)]; % greater than 50ms us
    trial_start = ind{1}(ind_pos);
    
    % table_start/cue pulse timestamps
    ind_pos = [1 1+find(diff(ind{2})>1e4)]; % greater than 1ms
    table_start = ind{2}(ind_pos);
    
    % laser pulse timestamps
    ind_pos = [1 1+find(diff(ind{3})>10e1)]; % greater than 10ms
    ind_Tpos = [1 1+find(diff(ind{3})>5e4)]; % greater than 5s
    laser = ind{3}(ind_pos);
    laser_start = ind{3}(ind_Tpos);
    
    % tone pulse timestamps
    ind_pos = [1 1+find(diff(ind{4})>1e4)]; % greater than 1s
    tone = ind{4}(ind_pos);
    
    % masking led light pulse timestamps
    ind_neg_neg = [1 1+find(diff(ind_neg{1})>1e4)]; % greater than 1s
...
elseif expt_type==2
    ...
    % camera timestamps
    % first timestamp = start of trial_start
    ind_pos = [1 1+find(diff(ind{1})>200e4)]; % greater than 50ms us
    trial_start = ind{1}(ind_pos);
    ...
    ind_pos = [1 1+find(diff(ind{4})>10e1)]; % greater than 10ms
    ind_Tpos = [1 1+find(diff(ind{4})>5e4)]; % greater than 5s
...
elseif expt_type==3
    ...
    % camera timestamps
    % first timestamp = start of trial_start
    ind_pos = [1 1+find(diff(ind{1})>0.05*sr)]; % greater than 50ms us
    trial_start = ind{1}(ind_pos);
    
    % table_start/cue pulse timestamps
    ind_pos = [1 1+find(diff(ind{2})>1*sr)]; % greater than 1ms
    table_start = ind{2}(ind_pos);
    
    % laser pulse timestamps
    ind_pos = [1 1+find(diff(ind{3})>0.01*sr)]; % greater than 10ms
    ind_Tpos = [1 1+find(diff(ind{3})>5*sr)]; % greater than 5s
    laser = ind{3}(ind_pos);
    laser_start = ind{3}(ind_Tpos);
    
    % laser gate pulse timestamps
    %     ind_pos = [1 1+find(diff(ind{4})>0.01*sr)]; % greater than 10ms
    %     ind_Tpos = [1 1+find(diff(ind{4})>5*sr)]; % greater than 5s
    laser_gate = [];%ind{4}(ind_pos);
    laser_gate_start = [];%ind{4}(ind_Tpos);
    %
    disp(['trial_start: ' num2str(length(trial_start))]);
