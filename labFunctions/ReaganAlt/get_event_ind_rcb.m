function get_event_ind_rcb(NEURAL,recsite,channels,expt_type,SAVE)
% INPUT
%       NEURAL: directory with neural data (String)
%       channels: channels 1-5, for sync pulse timestamps, camera timestamps
%                 table_start/cue pulse timestamps, laser pulse timestamps,
%                 laser gate, for grab detector trials (Vector)
...
% OUTPUT
%       event_ind: All variable for table moving
% DEPENDENCIES
...
fname = ls([NEURAL filesep recsite '\*.nidq.bin*']);
if isempty(fname)
    disp([fname '.bin not in folder'])
else
    % neural data meta file
    mname = ls([NEURAL filesep recsite filesep '*.nidq.meta*']);
    ...
    if expt_type==1
        ...
        if getSync
            ...
            [ind] = get_event_ind_jcr2([NEURAL filesep recsite filesep],fname,nchan,[ch2+1 ch3+1 ch4+1 ch5+1 ch1+1],[0.001 0.001 0.001 0.001 0.001],0);
            % sync pulse timestamps
            ind_pos = [1 1+find(diff(ind{5})>100e3/t_cal)]; % greater than 200ms
            sync = ind{5}(ind_pos);
        else
            [ind] = get_event_ind_jcr2([NEURAL filesep recsite],fname,nchan,[ch2+1 ch3+1 ch4+1 ch5+1],[0.001 0.001 0.001 0.001],0);
            sync=[];
        end
        ...
        if getLight
            %     [ind_neg, data_tmp_neg] = get_event_ind(neural_data_dirs,fname,nchan,[ch6+1],[-0.001],1);
            [ind_neg] = get_event_ind_jcr2([NEURAL filesep recsite],fname,nchan,[ch6+1],[-0.001],1);
        else
            light=[];
        end
    ...
    elseif expt_type==2
        ...
        if getSync
            %     [ind_neg, data_tmp_neg] = get_event_ind(neural_data_dirs,fname,nchan,[ch6+1],[-0.001],1);
            [ind] = get_event_ind_jcr2([NEURAL filesep recsite],fname(end,:),nchan,[ch2+1 ch3+1 ch4+1 ch5+1 ch1+1],[0.001 0.001 0.001 0.001 0.001],0);
            % sync pulse timestamps
            ind_pos = [1 1+find(diff(ind{5})>2e2)]; % greater than 200ms
            sync = ind{5}(ind_pos);
        else
            [ind] = get_event_ind_jcr2([NEURAL filesep recsite],fname(end,:),nchan,[ch2+1 ch3+1 ch4+1 ch5+1],[0.001 0.001 0.001 0.001],0);
            sync=[];
        end
    ...
    elseif expt_type==3
        ...
        [ind] = get_event_ind_jcr2([NEURAL filesep recsite],fname(end,:),nchan,[ch1+1 ch2+1 ch3+1],[0.001 0.001 0.001],0);
        sync=[];
...
saveData=1;
if saveData
    saveName = [SAVE filesep 'event_ind'];
    if expt_type==1
