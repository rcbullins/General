function create_trial_outcome_rcb(ROOT,NEURAL,VID,SAVE,...
    CALIB,expt_type,smoothwin,frames,LT_shift,CON_shift,plotfig)
...
%       VID: Directory with videos (String)
%       SAVE: Save directory (String)
%       CALIB: Directory of calibration file (String)
%       smoothwin: window to smooth for plotting (integer)
%       expt_type: Which recording setup it was done on (Integer)
%                   Jeremy = 1, Jay new = 2, Jay old = 3
%       frames: number of frames per trial (integer)
%       LT_shift:frame shift for laser trials
%       CON_shift: frame shift for baseline trials
%       plotfig: boolean (plot or not, 1 or 0)
% OUTPUT
...
%       trk files: output from apt, with movement over time
%%
saveData=1;
...
if expt_type==1
    ...
    [traj_1, conf_1] = get_traj_3D_RB([VID filesep 'tracked' filesep],CALIB,nframes,i_obj);
    i_obj = 2; % object number to track (1=2nd digit, 2=4th digit, 3=wrist, 4=pellet)
    [traj_2, conf_2] = get_traj_3D_RB([VID filesep 'tracked' filesep],CALIB,nframes,i_obj);
    i_obj = 3; % object number to track (1=2nd digit, 2=4th digit, 3=wrist, 4=pellet)
    [traj_3, conf_3] = get_traj_3D_RB([VID filesep 'tracked' filesep],CALIB,nframes,i_obj);
    i_obj = 4; % object number to track (1=2nd digit, 2=4th digit, 3=wrist, 4=pellet)
    [traj_4, conf_4] = get_traj_3D_RB([VID filesep 'tracked' filesep],CALIB,nframes,i_obj);
elseif expt_type==2
    i_obj = 1; % object number to track (1=2nd digit, 2=4th digit, 3=wrist, 4=pellet)
    [traj_1, conf_1] = get_traj_3D_RB([VID filesep 'tracked' filesep],CALIB,nframes,i_obj);
    i_obj = 2; % object number to track (1=2nd digit, 2=4th digit, 3=wrist, 4=pellet)
...
if strcmp(NEURAL,[ROOT '\jcr79\ephys\jcr79_20200930_4500_915um_2_g0'])
    vid_offset=28;
    frontList = dir(fullfile([VID '\tracked\'], '*front*.trk'));
    sideList = dir(fullfile([VID '\tracked\'], '*side*.trk'));
...
elseif strcmp(NEURAL,[ROOT '\jcr82\ephys\jcr82_20210101_4400um_g0'])
    vid_offset=-3;
    frontList = dir(fullfile([VID '\tracked\'], '*front*.trk'));
    sideList = dir(fullfile([VID '\tracked\'], '*side*.trk'));
...
elseif strcmp(NEURAL,[ROOT '\M341\ephys\M341_20210817_1000_g0_imec0'])
    trial_range=[1:120];
else
...
if expt_type==1
    if strcmp(NEURAL,'C:\Users\bullinsr\OneDrive - University of North Carolina at Chapel Hill\Hantman_Lab\ThalamusSP_Project\Data\jcr70\ephys\jcr70_20191016_920um_g0')
        accel_thresh=0.9e4; % 
    else
        accel_thresh=1e4; % 
    end
...
for t = trial_range
    ...
    if strcmp(NEURAL,[ROOT '\jcr82\ephys\jcr82_20210101_4400um_g0']) && t>102
        vid_offset=-3;
    else
        vid_offset=0;
    end
    ...
    if expt_type==1
        ...
        elseif strcmp(NEURAL,'C:\Users\bullinsr\OneDrive - University of North Carolina at Chapel Hill\Hantman_Lab\ThalamusSP_Project\Data\jcr70\ephys\jcr70_20191016_920um_g0')
    ...
    if plotfig
        close all;
        figure; set(gcf,'position',[1800 700 1500 600],'Name',NEURAL);
        hold on;
    ...
    if expt_type==1
        ...
        if strcmp(NEURAL,'D:\localData\jcr70\ephys\jcr70_20191016_920um_g0')
...
disp([NEURAL ' - create_trial_outcome complete'])