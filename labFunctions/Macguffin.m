  ...
  methods (Access=private)
    ...
    function initFromJLabelData(self,jld)
      ...
      if jld.gtMode ,
        ...
        self.expDirTags = jld.expdirtags;
      else
...
%       warningNoTrace('Macguffin:noTags','No experiment tags available.');
%       self.expDirTags = ExperimentTags.expTags(self.gtExpDirNames);
      self.addversion;
    end  % method
    ...
    function initFromMacguffins(self,m)
      ...
      FIELDSTHATMIGHTDIFFER = {'behaviors' 'file' 'labels' 'expDirNames' 'expDirTags' 'classifierStuff'};
...
%       classifierStuff.timeStamp=[];
  ...
  methods
    ...
    function verifyAug2014(self,jabfilename)
      % temporary method to check/convert multiclassifier jabs created
      % during dev phase
      
      if ~exist('jabfilename','var')
        jabfilename = '';
      end
      
      if ischar(self.file.scorefilename)
        self.file.scorefilename = {self.file.scorefilename};
      end
      behnames = Labels.verifyBehaviorNames(self.behaviors.names);
      nbeh = numel(behnames);
      nscr = numel(self.file.scorefilename);
      if nscr~=nbeh
        newsfns = cellfun(@(x)sprintf('scores_%s.mat',x),behnames,'uni',0);
        str = sprintf('Jabfile %s: Number of scorefiles (%d) does not match number of behaviors (%d). Using new scorefilenames: %s',...
          jabfilename,nscr,nbeh,civilizedStringFromCellArrayOfStrings(newsfns));
        uiwait(warndlg(str));
        self.file.scorefilename = newsfns;
      end
        
      if ~iscell(self.classifierStuff.params)
        self.classifierStuff.params = {self.classifierStuff.params};
      end
      if ~iscell(self.classifierStuff.trainingParams)
        self.classifierStuff.trainingParams = {self.classifierStuff.trainingParams};
      end
      assert(isequal(nbeh,numel(self.classifierStuff.params),...
        numel(self.classifierStuff.trainingParams)),'Jabfile corruption.');      
    end
    ...
    function modernize(self,dowarn)
      ...
      for i = 1:numel(self)
        ...
        self(i).expDirTags = ExperimentTags.cleanLegacyNotTags(self(i).expDirTags);
  ...
  methods (Static) % jabfile convenience methods
    ...
    function jabVerifyAug2014(jabfiles)
