function [traj conf] = get_traj_3D(data_dir,calib_file,n_frames,i_obj)
...
% List files.
fname_side = dir([data_dir '/tracked_side*.trk']);
clear foo
for i = 1:length(fname_side)
    foo(i) = str2num(fname_side(i).name(14:(end-4)));
end
[bar order] = sort(foo);
fname_side = fname_side(order);

fname_front = dir([data_dir '/tracked_front*.trk']);
clear foo
for i = 1:length(fname_front)
    foo(i) = str2num(fname_front(i).name(15:(end-4)));
end
[bar order] = sort(foo);
