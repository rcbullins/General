function [lfp] = getLFPfromBin(RAW_EPHYS_FILE,BASEPATH, SUB, EXPER_SESSION)
% PURPOSE
...
rec_length =  str2double(meta.fileTimeSecs);
% Get all lfp data
...
TMP_SAVE_FOLDER = [BASEPATH 'TMP_Data_Analyzed/' SUB '/Neural/tmp_mat_files/'];
if ~exist(TMP_SAVE_FOLDER)
...
for ichunk = 1:size(nSamp_chunk,2)-1
    ...
    if exist(lfp_chunk_file) == 0
        ...
        save([TMP_SAVE_FOLDER SUB '_' EXPER_SESSION '_lfpChunk' num2str(ichunk) '.mat'],'lfp_downsampled');
    else
        load(lfp_chunk_file);
    end
...
lfp_tmp = lfp_all;
lfp.data = lfp_tmp;
lfp.time = linspace(0,rec_length,size(lfp.data,2));
% delete the chunk files once all saved
for ichunkDel = 1:size(nSamp_chunk,2)-1
    delete([BASEPATH 'Data_Analyzed/' SUB '/Neural/tmp_mat_files/' SUB '_' EXPER_SESSION '_lfpChunk' num2str(ichunkDel) '.mat']);
end
end